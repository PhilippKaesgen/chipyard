...
make -j -C $SIM_DIR CONFIG=HiPReP${HIPREP_DIM}x${HIPREP_DIM}Rocket${TILES}Config VERILATOR_THREADS=8 timeout_cycles=${TIME_OUT} run-binary BINARY=$BIN_DIR/main_roofline.riscv || exit 1
